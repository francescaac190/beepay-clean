...
class TravelBloc extends Bloc<TravelEvent, TravelState> {
  ...
  TravelBloc({
    ...
    on<TravelPickOneDate>(
      (e, emit) => emit(state.copyWith(oneWayDate: e.date, clearRange: true)),
    );
    on<TravelPickRange>((e, emit) => emit(state.copyWith(
          rangeStart: e.start,
          rangeEnd: e.end,
          clearOneWay: true,
        )));
    on<TravelSetPassengers>(
      (e, emit) =>
          emit(state.copyWith(adults: e.adults, kids: e.kids, babies: e.babies)),
    );
    ...
    // ✅ Guardar vuelo seleccionado para InfoReserva
    on<TravelSelectFlight>((e, emit) {
      emit(state.copyWith(selectedFlight: e.flight));
    });
  }

  Future<void> _onLoadAirports(
      TravelLoadAirports e, Emitter<TravelState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(
          loading: false, error: 'No se pudieron cargar aeropuertos'));
    }
  }
  ...
  Future<void> _onSearchFlights(
      TravelSearchFlights e, Emitter<TravelState> emit) async {
    if (!state.isValid) {
      emit(state.copyWith(
          resultsError: 'Faltan datos para buscar', resultsLoading: false));
      return;
    }
    ...
    try {
      ...
      emit(state.copyWith(
          resultsLoading: false, resultsError: 'Error al obtener vuelos'));
    }
  }
  ...
  List<Flight> _sort(List<Flight> list, TravelSort s) {
    ...
    switch (s) {
      case TravelSort.priceAsc:
        flights.sort((a, b) =>
            _toDouble(a.totalAmount).compareTo(_toDouble(b.totalAmount)));
        break;
      case TravelSort.cashbackDesc:
        flights.sort(
            (a, b) => _toDouble(b.puntos).compareTo(_toDouble(a.puntos)));
        break;
